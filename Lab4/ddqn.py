...
if __name__ == "__main__":
    ...
    for step in range(total_timesteps):
        ...
        if random.random() < epsilon:
            ...
            with torch.no_grad():
                q_v = q_network(torch.from_numpy(obs).float().to(device))
                actions = q_v.argmax().item()
        ...
        if step > learning_starts:
            if step % train_frequency == 0:
                ...
                with torch.no_grad():
                    # ddqn: use online network to select action
                    next_q_values = q_network(next_buffer_obs)
                    next_q_values_target = target_network(next_buffer_obs)
                    
                    max_actions = next_q_values.argmax(dim=1, keepdim=True)
                    target_max = next_q_values_target.gather(1, max_actions)
    ...
    env.close()
    os.makedirs("Freeway-runs", exist_ok=True)
